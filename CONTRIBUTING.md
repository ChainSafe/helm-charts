# Contributing & tooling setup

This repo holds our shared Helm charts. This doc explains **every tool in the
stack** — what it does and when it runs — plus how to work locally.
...
ship a change you **bump `version` in `Chart.yaml`**. This is how a chart
actually becomes installable.
...
Drop a folder under `charts/` with its own `Chart.yaml`, `values.yaml`,
`templates/`, and `tests/`. CI (lint, unittest, kubeconform, Trivy) and
publishing pick it up automatically — nothing else to wire.
